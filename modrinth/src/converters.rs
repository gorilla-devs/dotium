...
pub fn project(raw: project_structs::Project) -> project::Project {
    project::Project {
        id: raw.id,
        slug: raw.slug,
        project_type: project_type(raw.project_type),
        name: raw.title,
        ...
        requirements: project::ProjectRequirements {
            server: project_support(raw.server_side),
            client: project_support(raw.client_side),
        },
        categories: raw.categories,
        downloads: raw.downloads,
        followers: raw.followers,
        icon: raw.icon_url.and_then(|i| Some(Asset::by_url(i.as_str()))),
        status: status(raw.status),
        published: raw.published,
        updated: raw.updated,
        created: raw.published,
        gallery: raw.gallery.into_iter().map(|i| gallery_item(i)).collect(),
        allows_distribution: raw.license.id != "custom".to_string()
            && raw.license.id != "arr".to_string(),
        team_id: raw.team,
    }
}
...
pub fn project_type(raw: project_structs::ProjectType) -> project::Type {
    match raw {
        project_structs::ProjectType::Mod => project::Type::Mod,
        project_structs::ProjectType::Modpack => project::Type::Modpack,
    }
}
...
pub fn donation_url(raw: project_structs::DonationLink) -> project::DonationLink {
